...
map_sample() {
    local R1="$1"
    local base_name=$(basename "$R1" _1_trimmed.fastq.gz)
    ...
    # Run STAR for single-end mapping
    STAR \
    --runThreadN $THREADS \
...
# Loop through each single-end read file and start mapping
for R1 in "$TRIM_DATA_DIR"/*_1_trimmed.fastq.gz; do
    map_sample "$R1"
done
...
COUNT_SCRIPT="$HOME/scripts/count_single.sh"
if [ -f "$COUNT_SCRIPT" ]; then
    cp "$COUNT_SCRIPT" "$PROJECT_DIR"
    sbatch "$PROJECT_DIR/count_single.sh" "$PROJECT_DIR"
else
    echo "Counting script not found: $COUNT_SCRIPT"
fi
